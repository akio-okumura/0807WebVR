...
public class WebVRManager : MonoBehaviour
{
    ...
    {
        // ここが呼ばれていない
        // WVM_text.GetComponent<Text>().text = "OnStartVR_called";
        Instance.setVrState(WebVRState.ENABLED);
        // 次のテキスト出来ない
        // WVM_text.GetComponent<Text>().text = "this.vrState : " + this.vrState;
    }
    ...
    void Update()
    {
        ...
        // OculusGoでVRモードからEscapeで抜けた時用

        if (Input.GetKeyDown(KeyCode.Escape) && this.vrState == WebVRState.NORMAL)
          OnEndVR();
    ...
    // webvr.jsからここの関数を呼び出すテストファンクション
    /* 実験用メソッドなので不要
    public void test_func()
    {
      WVM_text.GetComponent<Text>().text = "test_func_called";
    }
    */
}
